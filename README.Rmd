...
Error messages:
  ...
  * **Specify what input caused the error condition to occur.** In most cases, the first word should be the argument (enclosed in backticks) that is problematic. This part of the error message should be in the past tense, as if the function has come back from an attempt and realized that the inputs won't work. 
    ...
    Even if what caused the problem is still present when the error message is emitted, it should not be in the present tense. 
  
  * **They advise what the user needs to change** in the next sentence. Except when the error message is canonical (*e.g.* `missing, with no default`), it should contain clear instructions on what to do. 
